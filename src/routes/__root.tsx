...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "HomeLink Rwanda — Find homes for rent & sale" },
      { name: "description", content: "Rwanda's marketplace for rentals and properties for sale. Browse verified listings across all 30 districts and contact landlords directly." },
      { property: "og:title", content: "HomeLink Rwanda" },
      { property: "og:description", content: "Find rentals and properties for sale across Rwanda. Connect with landlords directly." },
...
function CacheInvalidator({ queryClient }: { queryClient: QueryClient }) {
  const router = useRouter();
  useEffect(() => {
    // Lazy import to avoid SSR window issues
